...
test('each mode keeps one save and a newer write replaces it', () => {
  clearGameStorage();
  assert.equal(saveGameToStorage(snapshot('A')), true);
...
test('classic and random-trade careers use completely separate single saves', () => {
  ...
  saveGameToStorage(snapshot('经典球员'), 'classic');
  saveGameToStorage(snapshot('平行联盟球员'), 'random_trade');

  assert.equal(loadGameFromStorage('classic')?.player?.name, '经典球员');
  assert.equal(loadGameFromStorage('random_trade')?.player?.name, '平行联盟球员');
